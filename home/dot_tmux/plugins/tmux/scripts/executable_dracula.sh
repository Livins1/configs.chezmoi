...
main()
{
  ...
  # message styling
  tmux set-option -g message-style "bg=${back_ground},fg=${white}"

  # status bar
  tmux set-option -g status-style "bg=${back_ground},fg=${white}"
  ...
  if $show_powerline; then
    ...
    tmux set-option -g status-left "#[bg=${background},fg=${cyan}]#{?client_prefix,#[bg=${background}],} ${left_icon}"
  fi
  ...
  tmux set-window-option -g window-status-format "#[fg=${white}]#[bg=${back_ground}] #I #W${flags}"
